...
from mathics.doc.md_doc import MarkdownMathicsDocumentation
...
DOC_MARKDOWN_FILE = os.environ.get("DOC_MARKDOWN_FILE", settings.DOC_MARKDOWN_FILE)
...
def write_markdown(
    ...
    documentation = MarkdownMathicsDocumentation()
    if not quiet:
        print(f"Writing LaTeX document to {DOC_MARKDOWN_FILE}")
    with open_ensure_dir(DOC_MARKDOWN_FILE, "wb") as doc:
    ...
    with open(DOC_VERSION_FILE, "w") as doc:
        doc.write("%% Mathics core version number created via doc2markdown.py\n\n")
