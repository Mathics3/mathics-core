...
"""
Low level notation structures. 
"""

from mathics.builtin.base import BoxExpression, BoxConstructError
...
class GridBox(BoxExpression):
    ...
    def get_array(self, elements, evaluation):
        ...
        list_lengths = tuple(len(row) for row in items)
        if any(item != list_lengths[0] for item in list_lengths[1:]):
            # if not is_constant_list():
            raise BoxConstructError
