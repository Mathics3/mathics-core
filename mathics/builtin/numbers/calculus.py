...
class Series(Builtin):
    """
     ...
     >> Series[Exp[x-y], {x, 0, 2}, {y, 0, 2}]
      = (1 - y + y ^ 2 / 2 + O[y] ^ 3) + (1 - y + y ^ 2 / 2 + O[y] ^ 3) x + (1 / 2 - y / 2 + 1 / 4 y ^ 2 + O[y] ^ 3) x ^ 2 + O[x] ^ 3
...
class SeriesCoefficient(Builtin):
    ...
    >> Series[Exp[Sin[x]], {x, 0, 5}]
     = 1 + x + x ^ 2 / 2 + (-1 / 8) x ^ 4 + (-1 / 15) x ^ 5 + O[x] ^ 6
...
class SeriesData(Builtin):
    ...
    def format_outputform(self, x, x0, data, nmin, nmax, den, evaluation: Evaluation):
        """SeriesData[x_, x0_, data_List, nmin_Integer, nmax_Integer, den_Integer]"""
        if x0.is_zero:
            variable = x
        else:
    ...
    def eval_makeboxes(
        ...
        form:StandardForm|TraditionalForm]"""
        # In the end this should not be needed...
        expansion = self.format_outputform(
            x, x0, data, nmin, nmax, den, form, evaluation
        )
